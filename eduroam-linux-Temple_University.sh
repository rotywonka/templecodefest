...
function setup_environment {
  ...
  URL="https://computerservices.temple.edu/eduroam"
  SUPPORT="help@temple.edu"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "This installer will only work properly if you are a member of ${bf}Temple University.${n}" "Continue" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:nac.temple.edu'})
        server_name = 'nac.temple.edu'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="nac.temple.edu"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
}
EOFW
